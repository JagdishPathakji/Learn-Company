...
-- SELECT + WHERE + GROUP BY + HAVING + ORDER BY + LIMIT (worst rated top 3 articles of 2026)
SELECT BIN_TO_UUID(article_id,1) AS article_id, AVG(rating_value) AS average_score 
FROM USER_RATING
...
-- Analyze ratings
SELECT 
    BIN_TO_UUID(article_id, 1) AS article_id,
    MIN(rating_value) AS lowest_rating,
    MAX(rating_value) AS highest_rating,
    AVG(rating_value) AS average_rating
FROM USER_RATING
GROUP BY article_id;

-- Find latest version number for particluar article
SELECT 
    BIN_TO_UUID(article_id, 1) AS article_id,
